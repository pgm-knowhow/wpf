...
既に照会した方法ではデータベースから取得した情報をDictionary<string, object>で保持していたが、それではobjectを必要に応じて調べてからキャストする必要があったりして分かりにくいので、テーブルの型を模したEntityクラスにデータベースから取得したデータを入れて保持する方法がある
...
C#は標準でリフレクションが利用できるので、Entity毎・プロパティ毎にデータ入力する処理を実装しなくても良い
